...
export default function EmployeeTargetTable({ tableRows, tableColumn }) {
  ...
  const handleDeleteUser = (userId) => {
    dispatch({ type: sagaActions.DELETE_EMPLOYEE_TARGET, userId });
    const deleteUser = userList.filter((user) => user.id !== userId);
    setSelected([]);
    setUserList(deleteUser);
  };
  const handleDeleteMultiUser = (selected) => {
    dispatch({ type: sagaActions.DELETE_EMPLOYEE_TARGET, selected });
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="left" sx={{ padding: '5px' }}>
                          ...
                          />
                          <Box component={RouterLink} to={`${PATH_DASHBOARD.eCommerce.editById}/${paramCase(id)}/edit`}>
